...
function module.draw(draw, state)
    ...
    drawRoom(draw, state, biome.rooms.Arachne)
    ...
    drawRoom(draw, state, biome.minibosses.Treant)
    drawRoom(draw, state, biome.minibosses.FogEmitter)
    drawRoom(draw, state, biome.minibosses.Assassin)
    imgui.Spacing()
    return true
end
